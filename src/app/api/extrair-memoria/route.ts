...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const dadosExtraidos = await page.evaluate(() => {
      ...
      try {
        ...
        const resultado = {
          ...
          funcUso: biodata.funcUso !== undefined ? biodata.funcUso.toString() : '',
        ...
        // Extrai funcUso (dentro de BIODATA)
        const funcUso = allText.match(/"BIODATA"[^}]*"funcUso"\s*:\s*(\d+)/i);
        const funcUsoValue = funcUso ? funcUso[1] : '';
    ...
    if (typeof dadosExtraidos === 'object' && dadosExtraidos !== null && 'found' in dadosExtraidos && !dadosExtraidos.found) {
      ...
      const sampleText = (dadosExtraidos as any).sampleText;
      console.log('Debug - Texto da página (primeiros 2000 chars):', sampleText);
    ...
    if (typeof dadosExtraidos === 'object' && dadosExtraidos !== null && 'found' in dadosExtraidos && dadosExtraidos.found === true) {
      const dados = dadosExtraidos as any;
      return NextResponse.json({
        success: true,
        ...
        kernel: dados.kernel || '',
        versao: dados.versao || '',
        versaoAS: dados.versaoAS || '',
        memoria: dados.memoria || '',
        funcUso: dados.funcUso || '',
        bioUso: dados.bioUso || '',
      });
    }
